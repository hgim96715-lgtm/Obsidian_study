...
---
# API 함수 작성 패턴 ⭐️⭐️⭐️⭐️

> **위치:** 맨 위 ❌ · `fetchAPI` / `authFetchApi` / Void를 안 뒤에 둔다. (도구 → 조립 → 아래는 리팩터 예시)

```txt
규칙:
  엔드포인트 1개 = 함수 1개
  authFetchApi<T>   → 응답 body가 있는 요청 (GET · POST · PATCH)
  authFetchApiVoid  → 204처럼 응답 body가 없는 요청 (DELETE · 일부 POST)
  정렬·권한·비즈니스 = 서버 / 클라이언트 함수는 경로·타입·method만
```

---
# 조립 공식 — 빈칸 채우기 ⭐️⭐️⭐️⭐️

Nest (또는 OpenAPI)에서 읽을 것:

| 읽을 것 | 예 | 함수에 들어가는 곳 |
|---------|----|-------------------|
| Method | `GET` | GET이면 method 생략 · POST면 `method: 'POST'` |
| Path | `/items/:id/subs` | 백틱 문자열 · `:id` → `${id}` |
| Auth | JWT 필요? | `authFetchApi` vs 공개 `fetchAPI` |
| 응답 | `SubItem[]` JSON | `Promise<ApiSubItem[]>` · 제네릭 `T` 동일 |
| 요청 body | 없음 / 있음 | 없으면 끝 · 있으면 `body: JSON.stringify(...)` |

빈 템플릿 (가장 흔한 GET + JWT + JSON):

```typescript
export function 동사명사(경로변수: string): Promise<응답타입> {
  return authFetchApi<응답타입>(`/고정경로/${경로변수}/나머지`);
}
```
...
```txt
1. 경로변수   = Nest :id 이름 그대로 (itemId, parentId …)
2. 응답타입   = Api*  (wire JSON과 필드 맞추기 · 배열이면 ApiX[])
3. 문자열 path = Nest path에서 :param → ${param}
4. return authFetchApi<응답타입>(path)  ← 여기서 끝
   (정렬·필터·권한 if문은 넣지 않음)
```

예시 — `GET /items/:itemId/subs` · 로그인 · 응답 `Sub[]`:

```typescript
// 1 경로변수 = itemId
// 2 응답타입 = ApiSub[]
// 3 path     = `/items/${itemId}/subs`
export function fetchItemSubs(itemId: string): Promise<ApiSub[]> {
  return authFetchApi<ApiSub[]>(`/items/${itemId}/subs`);
}
```
...
```typescript
export function createItem(body: CreateItemBody): Promise<ApiItem> {
  return authFetchApi<ApiItem>('/items', {
    method:  'POST',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(body),
  });
...
```txt
막히면: Nest path · 응답 JSON 한 줄 · 로그인 여부 세 가지만 적고
위 표에 꽂으면 함수가 나온다. 아래 절은 케이스별 완성본.
```

## 목록 조회 — GET /resources

```typescript
export function fetchItems(): Promise<ApiItem[]> {
  return authFetchApi<ApiItem[]>('/items');
}

// 쿼리스트링이 있을 때
export function fetchItems(params: { page?: number; keyword?: string }) {
  const sp = new URLSearchParams();
  if (params.page)    sp.set('page', String(params.page));
  if (params.keyword) sp.set('keyword', params.keyword);
  return authFetchApi<ApiItem[]>(`/items?${sp.toString()}`);
}
```
...
export function fetchItem(id: string): Promise<ApiItem> {
  return authFetchApi<ApiItem>(`/items/${id}`);
}
```

## 중첩 리소스 조회 — GET /resources/:id/sub


```typescript
// 특정 리소스에 속한 하위 리소스 목록
export function fetchSubItems(parentId: string): Promise<ApiSubItem[]> {
  return authFetchApi<ApiSubItem[]>(`/items/${parentId}/sub-items`);
}
```
...
```typescript
export function updateItem(id: string, body: UpdateItemBody): Promise<ApiItem> {
  return authFetchApi<ApiItem>(`/items/${id}`, {
    method:  'PATCH',
    headers: { 'Content-Type': 'application/json' },
    body:    JSON.stringify(body),
...
## 삭제 — DELETE /resources/:id → 204

```typescript
export function deleteItem(id: string): Promise<void> {
  return authFetchApiVoid(`/items/${id}`, { method: 'DELETE' });
}
```

## 단순 액션 — POST /resources/:id/action

```typescript
// body 없는 POST (join, leave, like 등 단순 실행)
export function joinItem(id: string): Promise<ApiItem> {
  return authFetchApi<ApiItem>(`/items/${id}/join`, { method: 'POST' });
}

// 응답도 없는 POST → void
export function leaveItem(id: string): Promise<void> {
  return authFetchApiVoid(`/items/${id}/leave`, { method: 'POST' });
}
```
...
|상황|함수|이유|
|---|---|---|
|데이터 조회|`authFetchApi<T>`|JSON body 있음|
|생성 / 수정|`authFetchApi<T>`|JSON body 있음|
|삭제 (204)|`authFetchApiVoid`|body 없음, 파싱 시 SyntaxError|
|단순 액션 (204)|`authFetchApiVoid`|body 없음|
|body 있는 POST, 응답 있음|`authFetchApi<T>` + headers + JSON.stringify|JSON 전송|
|body 없는 POST|`authFetchApi<T>` 또는 `Void` + method만|Content-Type 불필요|


```txt
Content-Type: 'application/json' 이 필요한 경우:
  JSON.stringify(body)를 보낼 때만
  body가 없는 POST나 DELETE는 헤더 불필요

URL 파라미터:
  경로 파라미터: `/items/${id}`        → 특정 리소스 식별
  쿼리스트링:   `/items?page=1`        → 필터/페이지네이션
  URLSearchParams로 조립 → [[JS_URL_Encoding]]
```

## 타입 이름 관행

```typescript
type ApiItem         = { id: string; name: string; ... }   // 응답 — Api 접두사
type CreateItemBody  = { name: string; ... }               // 요청 body — Body 접미사
type UpdateItemBody  = Partial<CreateItemBody>             // PATCH용
```

```txt
Api 접두사 = 서버에서 그대로 온 형태
UI에서 가공이 필요하면 mapper를 거쳐 다른 타입으로 변환
타입 설계 → [[NextJS_UI_Types]] / [[NextJS_ApiTypes_Mapper]]
```
---
