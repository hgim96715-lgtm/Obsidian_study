...
related:
  ...
  - "[[NextJS_UI_Types]]"
---
# NextJS_ApiTypes_Mapper — API 타입 정의 + 매퍼 함수
...
```mermaid-beautiful
flowchart LR
    API["API JSON<br/>Swagger Try it out"] --> WIRE["apiTypes.ts<br/>Api* 그대로"]
    WIRE --> MAP["mapper<br/>복사 · 집계 · 임시값"]
    UI["types.ts<br/>Post"] -.->|변환 목표| MAP
    MAP --> OUT["Post · UI props"]
```

```txt
의존 방향: apiTypes → mapper → types — apiTypes는 types import ❌
apiTypes = 응답만 · 요청(login 등)은 api.ts 인자로 충분하면 생략
mapPost(단건) 만든 뒤 mapPosts(목록)는 .map()으로 재사용
```
