...
## ⚠️ Set은 "참조"로 중복을 판단함 (객체/Date 주의) ⭐️⭐️⭐️⭐️
...
이유: Set은 SameValueZero 비교 알고리즘을 씀
  ...
  → 객체/배열의 === 비교가 "참조"로 일어난다는 것은 [[JS_Operators]] 참고
    Set의 중복 판단도 같은 원리
...
map.forEach((value, key) => console.log(key, value));  // 주의: value가 첫 번째
...
```txt
이 카운팅 패턴을 "미리 0으로 채운 고정 키 목록" 위에서 하면
날짜별/시간대별 통계처럼 데이터가 없는 구간도 0으로 보여줘야 하는 차트용 집계가 됨

Map을 "마지막 실행 시각"을 기억하는 스로틀링 저장소로 쓰는 패턴은 [[NestJS_Throttle]] 참고
```

## 실전 — 배열 → Map ID 인덱싱 패턴 ⭐️⭐️⭐️⭐️
...
// Map { 'a1' → { id: 'a1', title: '카드1' }, 'a2' → { id: 'a2', ... } }

byId.get('a1');  // { id: 'a1', title: '카드1' } — O(1)
```

```txt
new Map(배열.map(item => [item.id, item])) 읽는 법:
  ① cards.map((c) => [c.id, c])
     각 요소를 [키, 값] 쌍으로 변환
     → [['a1', {...}], ['a2', {...}]]
  ② new Map([[...], [...]])
     [키, 값] 쌍 배열을 받아서 Map 생성

왜 쓰는가:
  배열: cards.find(c => c.id === id) → O(n) 전체 순회
  Map:  byId.get(id)                 → O(1) 즉시 반환
  소켓으로 업데이트가 빈번한 목록에서 특히 유용
```

---

# WeakMap / WeakSet — 약한 참조 ⭐️⭐️⭐️
...
|비교 방식|SameValueZero (객체는 참조)|key도 동일|참조 비교|key 참조 비교|
|순회|가능|가능|불가능|불가능|
|size|있음|있음|없음|없음|
...
객체/Date 중복 제거    → 원시값(문자열 등)으로 변환 후 Set 사용 ⭐
key-value 저장        → Map (key가 객체/함수일 수도 있을 때 특히 유리)
배열 → ID 인덱스      → new Map(arr.map(x => [x.id, x])) — O(1) 탐색
