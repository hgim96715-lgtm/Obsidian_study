...
```mermaid-beautiful
flowchart LR
    HDR["Authorization<br/>Bearer 토큰"] --> JWT["JwtAuthGuard<br/>인증 · 서명 · 만료"]
    JWT -->|실패| E401[401]
    JWT -->|통과| USER["request.user<br/>payload · sub · role"]
    USER --> ROLE["RolesGuard<br/>인가 · @Roles"]
    ROLE -->|불일치| E403[403]
    ROLE -->|통과| DEC["@UserId() 등<br/>커스텀 데코레이터"]
    DEC --> CTRL[Controller]
```

```txt
인증(누구야?) → 인가(해도 돼?) — Guard를 나눠서 각각만 수정 가능
@Public() 라우트는 JwtAuthGuard만 건너뜀 — RolesGuard는 별도
```
