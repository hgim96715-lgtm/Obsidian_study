...
> [!info] 
> Auth.js는 OAuth/이메일/Credentials 로그인을 대신 처리해주는 인증 라이브러리고, DB Adapter(Prisma 등)를 쓰면 User/Account/VerificationToken이라는 정해진 모양의 테이블에 결과를 저장한다. `provider`+`providerAccountId`는 "어느 외부 서비스의 어떤 계정인지"를 식별하는 키, 토큰 필드들은 OAuth 인증 성공 시 그 서비스가 돌려준 값을 그대로 보관하는 곳이다.
...
> [!info] 
> 최신 상태(알아두기): 2025년 9월부터 Auth.js는 Better Auth 팀이 맡아서 유지보수 모드로 운영 중 — 보안 패치는 계속 들어오지만 신규 기능 개발은 멈춘 상태. 새로 시작하는 프로젝트엔 보통 Better Auth가 권장되지만, 이미 Auth.js v5로 구축된 프로젝트라면 급하게 옮길 필요는 없음.
...
# Auth.js Adapter 전체가 항상 필요한 건 아니다 — 아키텍처별 선택 ⭐️⭐️

```txt
아래에 나올 User/Account/VerificationToken 표준 스키마는
"Auth.js 자신이 세션과 유저 데이터를 직접 들고 있는" 경우를 가정한 것임
→ 모든 프로젝트가 이 모양을 그대로 가져갈 필요는 없음, 아키텍처에 따라 다름
```

|아키텍처|특징|Adapter 표준 스키마가 필요한가|
|---|---|---|
|A. 프론트가 세션/유저를 직접 관리|Next.js(등)가 Auth.js로 로그인 처리하고, 그 결과를 자기 DB에 직접 저장|예 — User/Account/VerificationToken 표준을 그대로 따르는 게 합리적|
|B. 백엔드가 인증·User·DB를 전부 소유, 프론트는 API 클라이언트|NestJS 등 백엔드가 User/Account 같은 도메인 모델을 자체 설계하고, 프론트는 그 백엔드가 발급한 토큰(Bearer)으로 fetch만 함|아니요 — Adapter 전체를 복사할 필요 없음|

```txt
B 패턴에서 "얇은 연결 테이블"이 의미하는 것:
  Auth.js Account 모델의 10개 필드(type/refresh_token/access_token/...)를 다 가져오는 게 아니라
  "이 유저가 어느 외부 서비스의 어떤 계정과 연결되어 있는지"만 기록하는 최소 테이블

  최소 구성 예시:
    userId             — 우리 User 테이블의 id
    provider           — "google" 등
    providerAccountId  — 그 서비스 안에서의 고유 ID
    @@unique([provider, providerAccountId])  — 중복 연동 방지

  access_token/refresh_token 같은 필드는 "그 토큰으로 우리가 직접 그 서비스의 API를
  나중에 다시 호출할 일이 있을 때"만 필요함 — 단순히 "로그인 수단으로만 쓴다"면
  로그인 성공 여부만 확인하고 토큰 자체는 굳이 안 저장해도 되는 경우가 많음
```

```txt
신규 프로젝트라면 — Auth.js가 유지보수 모드인 점까지 고려해서
Better Auth(자체 호스팅, 기능 풍부) / Clerk(매니지드, 빠른 셋업) 같은 대안도
함께 검토하는 흐름이 일반적 (위 "최신 상태" 콜아웃 참고)
```

---

# 표준 스키마 vs 이 프로젝트가 커스터마이즈한 부분

```txt
⚠️ 이 섹션부터는 "Adapter를 실제로 쓰기로 했을 때" 기준 설명임
   위 "아키텍처별 선택"에서 B 패턴(백엔드가 인증 전부 소유)으로 간다면
   이 표 자체보다는 바로 위 "얇은 연결 테이블" 쪽이 더 맞을 수 있음
```
...
| Adapter가 필요한 경우                                                                       | 프론트(Next.js 등)가 직접 세션/유저 DB를 관리할 때 (아키텍처 A)                                                                |
| Adapter가 필요 없는 경우                                                                     | 백엔드가 인증·User·DB를 전부 소유, 프론트는 Bearer fetch만 할 때 (아키텍처 B) — `provider`+`providerAccountId` 정도의 얇은 테이블만 직접 설계 |
